...
from magma_v2.agents.model_worker import build_grounded_model_handoff_from_artifacts, model_worker, write_model_handoff
from magma_v2.agents.model_worker_validator import model_worker_validator
...
from magma_v2.runtime.validation import build_model_validation_bundle, normalize_validation_report
...
async def call_model_worker(instructions: str, tool_context: object | None = None) -> dict:
    ...
    result = await _run_agent(model_worker, instructions, tool_context, "call_model_worker")
    run_dir = _run_dir(tool_context)
    return _ensure_stage_outcome(
        run_dir=run_dir,
        result=result,
...
@tool(context=True)
async def call_model_worker_validator(instructions: str = "", tool_context: object | None = None) -> dict:
    """Run the model-worker validator."""
    run_dir = _run_dir(tool_context)
    report_path = os.path.join(run_dir, "MODEL_VALIDATION_REPORT.json")
    try:
        report = normalize_validation_report(build_model_validation_bundle(run_dir), stage="model")
        write_json(report_path, report)
        log_action(
            run_dir,
            "orchestrator",
            "call_model_worker_validator",
            output_refs=[report_path],
            metadata={"mode": "deterministic"},
        )
        return {"status": "success", "model_validation_report_path": report_path, "report": report}
    except Exception as exc:
        log_action(
            run_dir,
            "orchestrator",
            "model_worker_validator_deterministic_failed",
            success=False,
            error=str(exc),
        )

    result = await _run_agent(model_worker_validator, instructions, tool_context, "call_model_worker_validator")
    report_path = os.path.join(run_dir, "MODEL_VALIDATION_REPORT.json")
    if os.path.exists(report_path):
        return {**result, "model_validation_report_path": report_path}
    log_action(
        run_dir,
        "orchestrator",
        "model_worker_validator_missing_report",
        success=True,
        error="MODEL_VALIDATION_REPORT.json was not produced.",
        metadata={"tool_errors": result.get("tool_errors", [])},
    )
    return {**result, "status": "warning", "warning": "MODEL_VALIDATION_REPORT.json was not produced."}
...
2. (A4 ablation: data_worker is disabled — no DATA_HANDOFF.json will be written.) Skip the data stage entirely. Proceed directly to step 3.
3. (A4 ablation: no data_worker_validator.) Call inspect_run_contract to check pipeline state. DATA_HANDOFF.json will be absent; this is expected and not an error. Proceed to model_worker regardless.
...
5. Call call_model_worker directly. model_worker must load and prepare the dataset, derive the target column, perform train/val/test splits, and handle all data logic itself using the user prompt and dataset path. There is no DATA_HANDOFF.json to read. Include the full user request and dataset path in the model instructions.
6. After MODEL_HANDOFF.json exists, call_model_worker_validator before calling judge.
7. call_model_worker deterministically writes MODEL_ERROR.json if the worker returns without MODEL_HANDOFF.json or MODEL_ERROR.json. Call inspect_run_contract. If MODEL_ERROR.json exists, call decide_repair_route and route by decision, error_class, retry_scope, and repair history:
   - path_resolution_error, split_error, leakage_error, join_error -> (A4 ablation: no data_worker available) instruct model_worker to fix data loading itself and retry.
